			...
			<div className="section-shell relative">
				<div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
					<motion.div
						...
						viewport={{ once: true }}
						className="rounded-3xl border border-rose-300/15 bg-[var(--bg-secondary)]/60 p-8 md:p-10"
					>
						...
						<p className="mt-4 text-[var(--cream)]/80">
							Limited bookings available this month. Reserve early for bridal
							dates, festive glam, and on-location events.
						</p>
						...
						<div className="mt-6 rounded-2xl border border-amber-200/25 bg-amber-300/10 p-4">
							<p className="inline-flex items-center gap-2 text-sm text-amber-100">
								<TriangleAlert size={16} /> Fast filling dates:{" "}
								{bookingOptions.fastFillingDates.join(", ")}
							</p>
						</div>
						...
						<div className="mt-7 flex flex-col justify-center gap-4 sm:flex-row sm:justify-start">
							<MagneticButton
								...
								target="_blank"
								rel="noreferrer"
								className="inline-flex items-center justify-center gap-2 rounded-full bg-[var(--gold)] px-7 py-3 font-semibold text-[var(--text-light)]"
								data-cursor="Book"
							>
							...
							<MagneticButton
								...
								target="_blank"
								rel="noreferrer"
								className="inline-flex items-center justify-center gap-2 rounded-full border border-rose-300/25 bg-white/8 px-7 py-3 font-medium text-[var(--text-light)]"
								data-cursor="Chat"
							>
					...
					<div className="grid gap-4 lg:grid-cols-[1fr_0.45fr]">
						<div className="rounded-3xl border border-rose-300/15 bg-[var(--bg-secondary)]/50 p-6 md:p-7">
							{submitted ? (
								<motion.div
								...
								>
									...
									<h3 className="mt-5 text-3xl text-[var(--cream)]">
										Booking Request Confirmed
									</h3>
									<p className="mt-3 max-w-md text-sm text-[var(--text-muted)]">
										Thank you, {formData.name || "beautiful"}! Our team will
										contact you shortly on {formData.phone} to finalize your
										slot.
									</p>
								</motion.div>
							) : (
								<>
											...
											<span
												...
												className={`rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.16em] ${
													step === index + 1
														...
														: "border border-rose-300/15 bg-white/5 text-[var(--text-muted)]"
												}`}
											>
												{index + 1}. {label}
											</span>
									...
									<div className="mt-6 min-h-[250px]">
										{step === 1 && (
											<div>
												<p className="text-sm uppercase tracking-[0.15em] text-[var(--soft-pink)]">
														...
														<button
															...
															className={`rounded-2xl border px-3 py-3 text-left text-sm transition ${
																formData.service === service
																	...
																	: "border-rose-300/10 bg-white/5 text-[var(--text-muted)] hover:border-rose-300/25"
															}`}
														>
															{service}
														</button>
										...
										{step === 2 && (
											<div>
												...
												<input
													...
													className="mt-3 w-full rounded-2xl border border-rose-300/10 bg-white/5 px-4 py-3 text-[var(--text-light)]"
												/>
											</div>
										)}
										...
										{step === 5 && (
											<div>
												...
												<div className="mt-3 rounded-2xl border border-rose-300/10 bg-white/5 p-4 text-sm text-[var(--cream)]/90">
													<p>
									...
									<div className="mt-6 flex flex-wrap gap-3">
										<button
											...
											onClick={onBack}
											disabled={step === 1}
											className="rounded-full border border-white/25 px-5 py-2 text-sm text-[var(--text-light)] disabled:opacity-50"
										>
											Back
										</button>
										...
										{step < steps.length ? (
											<MagneticButton
												...
												onClick={onNext}
												disabled={!canProceed}
												className="rounded-full bg-gradient-to-r from-[var(--gold)] to-[var(--rose-gold)] px-5 py-2 text-sm font-semibold text-[var(--text-light)] disabled:opacity-55"
											>
												Continue
											</MagneticButton>
										) : (
											<MagneticButton
												...
												type="button"
												onClick={onConfirm}
												className="rounded-full bg-emerald-500 px-5 py-2 text-sm font-semibold text-[var(--text-light)]"
											>
												Confirm Booking
											</MagneticButton>
										)}
						...
						<aside className="rounded-3xl border border-rose-300/15 bg-[var(--bg-secondary)]/50 p-5 lg:sticky lg:top-28 lg:h-fit">
							...
							<div className="mt-5 rounded-2xl border border-rose-300/10 bg-white/7 p-3 text-xs uppercase tracking-[0.16em] text-[var(--soft-pink)]">

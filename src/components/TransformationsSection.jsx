...
function BeforeAfterCard({ item, index }) {
	...
	return (
		<motion.article
			...
			className="rounded-3xl border border-pink-300/70 bg-white/60 backdrop-blur-md p-4 shadow-xl shadow-black/5"
		>
			<p className="mb-3 text-sm font-semibold uppercase tracking-[0.16em] text-[var(--text-muted)]/80">
				{item.category}
			</p>
			<div
			...
			>
				...
				<div className="absolute inset-y-0" style={{ left: `${position}%` }}>
					<span className="absolute -left-px h-full w-[2px] bg-pink-200/80 shadow-[0_0_16px_rgba(255,255,255,0.8)]" />
				</div>
				<span className="absolute left-3 top-3 rounded-full bg-[var(--bg-secondary)]/60 px-3 py-1 text-[10px] uppercase tracking-[0.14em] text-[var(--text-light)]">
					Before
				</span>
				<span className="absolute right-3 top-3 rounded-full bg-[var(--bg-secondary)]/60 px-3 py-1 text-[10px] uppercase tracking-[0.14em] text-[var(--text-light)]">
					After
				</span>
			</div>
			<input
			...
			/>
			<h3 className="mt-3 text-xl text-[var(--text-light)]">{item.title}</h3>
		</motion.article>
	);
}

		...
		<section className="relative overflow-hidden bg-[var(--cream)] py-24 text-[var(--text-light)] md:py-32">
			...
			<div className="section-shell grid items-center gap-10 lg:grid-cols-[0.9fr_1.1fr]">
				<motion.div
					...
					className="relative"
				>
					<div className="absolute -left-6 -top-6 rounded-2xl border border-pink-400/30 bg-white/60 backdrop-blur-md px-4 py-2 text-xs uppercase tracking-[0.2em] text-[var(--text-muted)] shadow-lg">
						Meet The Artist
					</div>
					...
					<div className="absolute -right-3 top-16 hidden w-36 gap-3 sm:grid">
						{artistProfile.gallery?.slice(0, 2).map((image, index) => (
							<div
								key={image}
								className="overflow-hidden rounded-2xl border border-pink-400/30 bg-white/60 backdrop-blur-md p-1 shadow-xl"
							>
								<SafeImage
									src={image}
					...
					<div className="absolute -bottom-6 right-6 rounded-2xl border border-pink-400/30 bg-white/95 p-4 shadow-xl backdrop-blur">
						<p className="text-3xl font-semibold text-[var(--text-light)]">5+ Years</p>
				...
				>
					...
					<div className="mt-6 grid gap-4 md:grid-cols-2">
						<div className="rounded-2xl border border-pink-300/80 bg-white/60 backdrop-blur-md p-5">
							<p className="inline-flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.15em] text-[var(--text-muted)]">
								<BadgeCheck size={14} className="text-[var(--rose-gold)]" />{" "}
								Specialties
							</p>
						...
						<div className="rounded-2xl border border-pink-300/80 bg-white/60 backdrop-blur-md p-5">
							<p className="inline-flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.15em] text-[var(--text-muted)]">
								<Award size={14} className="text-[var(--rose-gold)]" />{" "}
								Certifications
							</p>
							<ul className="mt-3 space-y-2 text-sm text-[var(--text-muted)]">
					...
					<blockquote className="mt-7 border-l-2 border-[var(--rose-gold)] pl-4 italic leading-relaxed text-[var(--text-muted)]">
						"{artistProfile.quote}"
					</blockquote>
					...
					<div className="mt-6 grid grid-cols-3 gap-3">
						{artistProfile.gallery?.map((image, index) => (
							<div
								key={`${image}-${index}`}
								className="overflow-hidden rounded-2xl border border-pink-300/90 bg-white/60 backdrop-blur-md p-1"
							>
								<SafeImage
									src={image}
									fallbackSrc={artistProfile.image}
					...
					<p className="mt-4 text-sm font-semibold uppercase tracking-[0.3em] text-[var(--text-muted)]/80">

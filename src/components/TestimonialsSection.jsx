		...
		>
			...
			<div className="section-shell">
				<motion.div
					...
					className="mb-8 rounded-2xl border border-rose-300/10 bg-gradient-to-r from-white/10 to-white/5 px-5 py-4 text-center text-sm text-[var(--cream)]"
				>
				...
				<p className="mx-auto mt-4 max-w-3xl text-center text-[var(--text-muted)]">
					Genuine stories from bridal, engagement, and party clients who trusted
				...
				>
					{testimonials.map((item) => (
						<SwiperSlide key={item.name}>
							<article className="hover-lift-card h-full rounded-3xl border border-rose-300/10 bg-white/7 p-6">
								...
								<div className="mt-6 flex items-center gap-3">
									...
									<div>
										...
										<p className="text-xs uppercase tracking-[0.16em] text-[var(--soft-pink)]">
											{item.type}
										</p>
									</div>
								</div>
								...
								{item.before && item.after && (
									<div className="mt-4 grid grid-cols-2 gap-2">
										<div className="overflow-hidden rounded-xl border border-rose-300/10">
											<SafeImage
												src={item.before}
												fallbackSrc={imageFallback}
											...
											/>
											<p className="px-2 py-1 text-[9px] uppercase tracking-[0.16em] text-[var(--text-muted)]">
												Before
											</p>
										</div>
										<div className="overflow-hidden rounded-xl border border-rose-300/10">
				...
				<div className="mt-10 grid gap-4 lg:grid-cols-3">
					{googleReviews.map((review, index) => (
						<motion.article
							...
							className="rounded-2xl border border-rose-300/10 bg-[var(--bg-secondary)]/40 p-5"
						>
				...
				<div className="mt-8 flex flex-col items-center gap-3 rounded-3xl border border-rose-300/10 bg-white/5 p-6 text-center sm:flex-row sm:justify-between sm:text-left">
					<div>
						<p className="inline-flex items-center gap-2 text-sm font-medium text-[var(--cream)]">
							<Sparkles size={15} className="text-[var(--gold)]" /> Average
							Rating 4.9 / 5.0
						</p>
						<p className="mt-1 text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">
							Google | Instagram | WhatsApp Referrals
						</p>
					</div>
					<p className="inline-flex items-center gap-2 rounded-full border border-rose-300/15 bg-white/8 px-4 py-2 text-xs uppercase tracking-[0.2em] text-[var(--cream)]">
						<BadgeCheck size={14} className="text-[var(--gold)]" /> Verified
						Client Feedback
					</p>
				</div>

				<div className="mt-10 rounded-3xl border border-white/12 bg-[var(--bg-secondary)]/60 p-6">
					<p className="text-center text-xs uppercase tracking-[0.22em] text-[var(--soft-pink)]">
						Featured In
					</p>
